...
<div class="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm max-w-4x1">
    <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-6">
            <div class="col-span-2">
                ...
                <input type="text" name="title" value="{{ old('title', $event->title) }}" class="w-full px-5 py-3 rounded-xl border border-slate-200 outline-none" required>
            </div>
            ...
            <div class="col-span-2">
                <label class="block text-sm font-bold text-slate-700 mb-2 uppercase">Poster Event</label>
                <div class="flex items-start gap-6 p-4 border border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                    <div class="shrink-0 text-center">
                        <p class="text-[10px] font-black uppercase text-slate-400 mb-2">Poster Saat Ini</p>
                        <img src="{{ asset('storage/'.$event->poster_path) }}" class="w-24 h-32 rounded-xl object-cover shadow-md border-2 border-white" onerror="this.src='https://placehold.co/24x32'">
                    </div>
                    <div class="flex-1">
                        <p class="text-xs text-slate-500 mb-3">Pilih file baru jika ingin mengganti poster. Kosongkan jika tidak ingin mengubah gambar.</p>
        ...
        <div class="flex justify-end gap-4 mt-10 pt-6 border-t border-slate-100">
            ...
            <button type="submit" class="px-10 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg hover:bg-indigo-700 transition">Simpan Perubahan</button>
        </div>
    </form>
</div>
@endsection